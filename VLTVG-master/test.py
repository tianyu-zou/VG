...
from engine import train_one_epoch, train_one_epoch_w_accum, evaluate, data_prefetcher
...
def get_args_parser():
    ...
    parser.add_argument('--checkpoint', default='VLTVG_R101_unc.pth', help='resume from checkpoint')
    ...
    # configure file
    parser.add_argument('--config',default='configs/VLTVG_R50_gref.py', type=str, help='Path to the configure file.')
...
def main(args):
    ...
    results = []
    for _, (img, mask, target) in enumerate(tqdm(prefetcher)):
        ...
        # print(outputs['pred_boxes'][0][0].detach().cpu().numpy())
        x, y, w, h = outputs['pred_boxes'][0][0].detach().cpu().numpy()
        ...
        # 保存到 JSON 文件
    with open('VLTVG_predictions.json', 'w', encoding='utf-8') as f:
