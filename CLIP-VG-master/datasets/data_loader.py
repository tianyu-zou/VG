...
class TransVGDataset(data.Dataset):
    ...
    def __init__(self, data_root, split_root='data', dataset='referit',
        ...
        with open('../VG-RS-question.json', 'r', encoding='utf-8') as f:
            data = json.load(f)
        for item in data:
            full_path = item['image_path']
            bbox = item.get('bbox', None)  # 读取 bbox，如果没有可设置 None
            phrase = item.get('question', '')  # 读取文本
            self.images.append((full_path, bbox, phrase))
    ...
    def pull_item(self, idx):
        ...
        img_path = osp.join('../VG-RS-images', os.path.basename(img_file.replace('\\', '/')))
        img = Image.open(img_path).convert("RGB")
    ...
    def __getitem__(self, idx):
        ...
        text_token = clip.tokenize(phrase, truncate=True)  # 1*77
